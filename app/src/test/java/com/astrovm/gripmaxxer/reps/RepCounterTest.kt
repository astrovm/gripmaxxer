...
class RepCounterTest {
    ...
    @Test
    fun `pull-up mode disarms when grip is lost after returning to down`() {
        val counter = RepCounter(
            featureExtractor = PoseFeatureExtractor(),
            config = RepCounterConfig(
                stableMs = 40L,
        ...
        val noGripDownLikeFrame = buildFrame(
            noseY = 0.38f,
            leftShoulderY = 0.50f,
            rightShoulderY = 0.50f,
            leftElbow = NormalizedLandmark(0.45f, 0.43f),
            rightElbow = NormalizedLandmark(0.55f, 0.43f),
            leftWrist = NormalizedLandmark(0.53f, 0.52f),
            rightWrist = NormalizedLandmark(0.47f, 0.52f),
        )
        ...
        counter.process(frame = downFrame, hanging = true, nowMs = 360L)
        counter.process(frame = downFrame, hanging = true, nowMs = 440L)
        counter.process(frame = noGripDownLikeFrame, hanging = true, nowMs = 520L)
        counter.process(frame = upFrame, hanging = true, nowMs = 620L)
        val noGhostRep = counter.process(frame = upFrame, hanging = true, nowMs = 700L)
